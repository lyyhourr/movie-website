...
export const LandscapeSlideLayout = (
  movie: MovieType[],
  title: string,
  isTv?: boolean
) => (
    ...
    {movie.map(
      (item, i) =>
        item.poster_path && (
          <SwiperSlide key={i}>
            <Link
              className="group duration-500 cursor-pointer"
              href={`/movie/${item.id}`}
            >
              <Image
                src={`https://image.tmdb.org/t/p/w500/${item.poster_path}`}
                width={10000000}
                height={10000000}
                className="bg-transparent  sm:hover:scale-105 duration-500 h-[260px] sm:w-[220px]  sm:h-[320px] lg:w-[250px] lg:h-[350px] xl:w-[300px] xl:h-[420px]  rounded-xl object-fill mx-auto "
...
export const MenuMoviesLayout = (
  ...
  useEffect(() => {
    setIsLoading(true);
    fetch(
      `https://api.themoviedb.org/3/${movieType}?api_key=${api_key}&${api_query}&page=${page}`
    )
      .then((res) => res.json())
      .then((data) => {
        setMovies(data.results), setIsLoading(false);
      });
  }, [page]);
  useEffect(() => {
  ...
  return (
    <div className="mt-[70px] mx-auto text-center text-3xl">
      ...
      {!isLoading && (
        <>
          <p className="text-4xl my-5 sm:my-[50px] font-semibold md:font-bold">
            {" "}
            {title}{" "}
          </p>
